#!/bin/bash
# Deployment script for OpenAI LLM Service
...
# Configuration
CONTAINER_PREFIX="${CONTAINER_PREFIX:-openai-llm-service}"
IMAGE_NAME="openai-llm-service:latest"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Actions:"
            echo "  start      Start the OpenAI LLM service"
            echo "  stop       Stop the OpenAI LLM service"
            echo ""
            echo "Options:"
            echo "  --build         Build the Docker image before starting"
            echo "  --port PORT     Port to expose the service (default: 8011)"
...
stop_app() {
    if podman ps -a --filter "name=${CONTAINER_PREFIX}" --format "{{.Names}}" | grep -q "^${CONTAINER_PREFIX}$"; then
        log_info "Stopping OpenAI LLM container: ${CONTAINER_PREFIX}"
        podman stop "${CONTAINER_PREFIX}" >/dev/null 2>&1 || true
        podman rm "${CONTAINER_PREFIX}" >/dev/null 2>&1 || true
    else
        log_info "OpenAI LLM container not found, skipping"
    fi
}
...
start_app() {
    ...
    log_info "OpenAI LLM Service started"
...
# Main execution
log_info "========================================="
log_info "OpenAI LLM Service Deployment"
log_info "========================================="
