#!/bin/bash
# Deployment script for WordVoyage Backend
...
# Configuration
CONTAINER_PREFIX="${CONTAINER_PREFIX:-wordvoyage-backend}"
IMAGE_NAME="wordvoyage-backend:latest"
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo ""
            echo "Actions:"
            echo "  start      Start the WordVoyage backend"
            echo "  stop       Stop the WordVoyage backend"
            echo ""
            echo "Options:"
            echo "  --build         Build the Docker image before starting"
            echo "  --port PORT     Port to expose the service (default: 8080)"
...
stop_app() {
    if podman ps -a --filter "name=${CONTAINER_PREFIX}" --format "{{.Names}}" | grep -q "^${CONTAINER_PREFIX}$"; then
        log_info "Stopping WordVoyage backend container: ${CONTAINER_PREFIX}"
        podman stop "${CONTAINER_PREFIX}" >/dev/null 2>&1 || true
        podman rm "${CONTAINER_PREFIX}" >/dev/null 2>&1 || true
    else
        log_info "WordVoyage backend container not found, skipping"
    fi
}
...
start_app() {
    if [ $BUILD_IMAGE -eq 1 ]; then
        log_info "Building WordVoyage backend image..."
        bash "$SCRIPT_DIR/build.sh"
    fi

    log_info "Starting WordVoyage Backend Service..."
    ...
    log_info "WordVoyage Backend Service started"
...
# Main execution
log_info "========================================="
log_info "WordVoyage Backend Deployment"
log_info "========================================="
